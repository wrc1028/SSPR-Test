...
public class SSPR : MonoBehaviour
{
    ...
    public float stretchIntensity = 43.0f;
    ...
    private void Start()
    {
        ...
        //SSPRCS = AssetDatabase.LoadAssetAtPath<ComputeShader>(@"Assets\Raw\Scene\Models\Water\SSPR.compute");
        if (SSPRCS != null)
        {
            // 获得内核索引
    ...
    private void SetFrameParam()
    {
        ...
        if ((reflectParam.x != Screen.width / (int)textureSize || reflectParam.y != Screen.height / (int)textureSize) && isAddSSPRCMD) ClaenupBuffer();
    ...
    private void AddSSPRCMD()
    {
        ...
        SSPRResult = CreateRenderTexture((int)reflectParam.x, (int)reflectParam.y, 0, RenderTextureFormat.Default);
        ...
        CopyCameraColor = new CommandBuffer();
        CopyCameraColor.name = "CopyCameraColor";
    ...
    {
        cameraColorTexture.Release(); // if(cameraColorTexture != null) 
        SSPRResult.Release(); // if(SSPRResult != null) 
        // SSPRResultBlur.Release(); if(SSPRResultBlur != null) 
        SSPRBuffer.Release(); // if(SSPRBuffer != null) 
    ...
    {
        // cmd.SetComputeTextureParam(SSPRCS, ClearKernel, cameraColorTextureId, cameraColorTexture);
